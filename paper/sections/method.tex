...
\begin{figure*}[htbp]
  ...
  After identifying the relevant keywords of requirements, we match them with the textual attributes of the software development artifacts in the repository. We create a trace link when an artifact has at least one matching keyword. We record the link in our graph database by creating an \emph{tracesTo} between the requirement node and the artifact node.

  \paragraph{TF-IDF Vectors} We first build a corpus consisting of all words in the requirements and the textual properties of software development artifacts. Before processing, we remove the stop words from this corpus. Later on, we calculate the TF-IDF values of each word using Equations \ref{eq:tf}, \ref{eq:idf}, and \ref{eq:tfidf} and construct a TF-IDF vector for each requirement and artifact based on the words they include. We then link the requirements with artifacts that have vectors which has a similarity score higher than a threshold. In Sec.~\ref{sec:eval} we experiment with various threshold values. Figure~\ref{fig:tfidfvec} presents the steps for this method.
